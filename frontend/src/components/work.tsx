      ...
      <div className="relative flex flex-col w-full">
        ...
        <div className="flex flex-col w-full mt-10 relative z-10">
          {steps.map((step, index) => {
            ...
            return (
              <div key={index} className={`flex flex-col md:flex-row items-center w-full max-w-5xl mx-auto mb-24 md:mb-32 group relative ${isEven ? '' : 'md:flex-row-reverse'}`}>
                
                {/* Number Side */}
                <div className={`w-full md:w-1/2 flex justify-center ${isEven ? 'md:justify-end md:pr-16' : 'md:justify-start md:pl-16'} relative`}>
                  <span className={`font-sketch text-[120px] md:text-[180px] leading-none text-sketch-black/40 select-none drop-shadow-sm inline-block transform ${isEven ? '-rotate-3' : 'rotate-3'}`}>
                    {step.num}
                  </span>
                </div>
                
                {/* Content Side */}
                <div className={`w-full md:w-1/2 flex flex-col pt-4 md:pt-12 items-center text-center ${isEven ? 'md:items-start md:text-left md:pl-16' : 'md:items-end md:text-right md:pr-16'}`}>
                  <h3 className="font-sketch tracking-wide text-3xl md:text-5xl font-bold text-blueprint-blue mb-4">
                    {step.title}
                  </h3>
                  <div className="w-full max-w-md border-b-2 border-dashed border-sketch-black/30 mb-4"></div>
                  <p className="font-mono font-bold text-sketch-black mb-2 tracking-wide">
                    {step.duration}
                  </p>
                  <p className="font-mono text-sketch-black/80 leading-relaxed max-w-md">
                    {step.desc}
                  </p>
                </div>
